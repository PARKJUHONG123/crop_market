...
String query = "";
	...
	try{
		...
		do{
			...
			query = "SELECT * " +
					...
					"WHERE Order_number = ?";
			pstmt = conn.prepareStatement(query);
			order_num = o + Integer.parseInt(today);
	...
	<script>
			...
			System.out.println(id+" "+order_num+" "+todayformat+" "+Rnum);
			...
			System.out.println("33WEWRERWER3");
			query = "SELECT distinct R.Retailer_number, R.Retailer_name, C.Address "+
					"FROM SHOPPINGBAG B, CUSTOMER C, RETAILER R, HAS_A H, ITEM I  "+
					"WHERE B.Customer_id = C.Id  "+
					"AND H.Ino = B.Purchase_item  "+
					"AND H.Ino = I.Item_number  "+
			...
			System.out.println("3DDDDDDDDDDDDDDDDDD");
			int Rnum = 0;
			int stock = 0;
			int quan = 0;
			String inum = "";
			...
			while(rs.next()){//매장 정보 받아오기
				Rnum = rs.getInt("Retailer_number");
			}
			query = "";
			DBUtil.close(pstmt1);pstmt1=null;
			DBUtil.close(rs); rs=null;
			...
			DBUtil.close(pstmt3);pstmt3=null;
			/**/
			System.out.println("dsfdddddddddddd");
			/*구매가능한 제품은 근처매장에서 구입하기*/
			query = "SELECT distinct R.Retailer_number, R.Retailer_name, C.Address, H.Stock, B.Quantity, I.Item_number "+
					"FROM SHOPPINGBAG B, CUSTOMER C, RETAILER R, HAS_A H, ITEM I  "+
					"WHERE B.Customer_id = C.Id  "+
					"AND H.Ino = B.Purchase_item  "+
					"AND H.Ino = I.Item_number  "+
			...
			pstmt1.setString(1, id);
			rs = pstmt1.executeQuery();
			System.out.println("dafewfaszdf");
			while(rs.next())
			{
				/*구매한 매장 재고 깎기*/
				System.out.println("-------------");
